...
- **Product Library** — data about products, held or not. ETF/Fund/Index/Stock facts, historical prices, scorecards, classification. Can be public — this is the part of the old prototype worth the most, and none of it touches your real holdings.
- **Simulator** — a virtual layer: `My Portfolio + Product Library → Simulation`. Never writes to the real ledger.
...
| "Held by me" vs "Research only" | none directly — new framing from this conversation | Product Library table above | none new — computed: `exists(select 1 from transactions where security_id = ...)` | Product Library page, filter/badge | ★★★★ |
| Scorecards (9-dimension, 0-100) | `productScores()` — costs/tax/diversification/liquidity/transparency/scale/efficiency/risk/return | Product Library table, Daily Prices (for vol/return-based dimensions) | Computed on read (Intelligence) — optionally cached in a `security_scorecards` view | Product detail page | ★★★★ |
...
| Feature | Existing code | Dependencies | Supabase table(s) | New module | Priority |
|---|---|---|---|---|---|
| 12-question profiling → 10D Investor DNA → Top-3 personalised portfolios, reasoning + trade-offs | `PROFILE_Q`, `computeProfileVector`, `scoreProduct`, `buildOneStrategy`, `computeSuitability`, `generateTop3`, `buildReasoning`, `buildInsights` — already in English | Product Library (operates only on the product universe — never reads real holdings, which is exactly what makes it public-safe) | None required — stateless questionnaire. Optional `profile_sessions` if you want to persist visitor answers | **New** public page (`discover.html`, or a tab on Product Library) | ★★★★ |
...
| Feature | Existing code | Dependencies | Supabase table(s) | New module | Priority |
|---|---|---|---|---|---|
| Real AI portfolio assistant ("why did my portfolio fall today?", "what's my US exposure?") | **none reusable** — old version is a Chatbase embed with no custom logic, no data access | Platform Architecture's AI principle (reads validated data, proposes Commands, never writes — already decided, not new); ideally Simulator + Product Library exist first for richer answers | None new — reads existing derived views | A chat surface using the Claude API directly, not Chatbase | ★★★ |

---

## Suggested execution order

1. **Daily Prices** — highest leverage, smallest surface area, everything else in Product Library/Simulator gets better once real prices exist.
2. **Product Library** (extended `securities` fields + the list/detail page) — Daily Prices needs somewhere to attach to; this is that somewhere.
3. **Scorecards** — cheap once Product Library exists, mostly porting `productScores()`.
4. **Simulator** — the biggest single feature, but the math is close to a direct port; do this once Product Library gives it real products to add.
5. **Investor DNA / Recommendation Engine** — independent of the above once Product Library exists; good candidate to build in parallel if you want two threads going.
